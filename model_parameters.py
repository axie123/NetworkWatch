...
class CNN:
    ...
    def kernel_filter(self, conv_layer, prev_filter_num, filter_group_num):
        # View the kernel filter.
        filter = self.kernels[conv_layer][filter_group_num-1]\
        [prev_filter_num-1]
        return filter

    def kernel_filter_range(self, conv_layer, prev_filter_num_start,
                            prev_filter_num_end, filter_group_num):
        # Gives a range of kernel filters.
        filter_range = self.kernels[conv_layer][filter_group_num-1]\
    ...
    def fc_node_to_node_weight(self, start_node, end_layer, end_node):
        # Weight going between two nodes.
        e_layer = self.weights[end_layer]
        n_to_n_weight = e_layer[end_node - 1][start_node - 1]
        return n_to_n_weight
